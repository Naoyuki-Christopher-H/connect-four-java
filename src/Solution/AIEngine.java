...
public class AIEngine
{
    ...
    public int findBestMove(char player) throws IllegalStateException
    {
        if (model.isBoardFull())
        {
            throw new IllegalStateException("Board is already full");
        }
    ...
    private int evaluateHorizontal()
    {
        ...
        for (int row = 0; row < GameModel.ROWS; row++)
        {
            ...
            {
                score += evaluateSegment(board, row, col, 0, 1);
            }
        }
        return score;
    }
    ...
    private int evaluateVertical()
    {
        ...
        for (int col = 0; col < GameModel.COLS; col++)
        {
            ...
            {
                score += evaluateSegment(board, row, col, 1, 0);
            }
        }
        return score;
    }
    ...
    private int evaluateDiagonals()
    {
        ...
        for (int row = 0; row <= GameModel.ROWS - 4; row++)
        {
            for (int col = 0; col <= GameModel.COLS - 4; col++)
            {
                score += evaluateSegment(board, row, col, 1, 1);
            }
        }
    ...
    private int evaluateSegment(char[][] board, int row, int col, int rowStep, int colStep)
    {
        int redCount = 0;
        int blackCount = 0;
